...
const Flashcards = ({ temaId, onVolver }) => {
  ...
  const renderInstructions = () => (
    <Card className="mb-4 border-2 border-sociologia-300 bg-gradient-to-br from-sociologia-50 to-sociologia-100 shadow-lg">
      <CardContent className="p-6">
        <h2 className="flex items-center text-2xl font-bold text-sociologia-700 mb-4">
          <HelpCircle className="mr-2" />
          Instrucciones para las FlashCards
        </h2>
        <ol className="list-decimal list-inside space-y-3 text-sociologia-600">
        ...
        <Button 
          ...
          className="mt-6 bg-sociologia-600 hover:bg-sociologia-700 text-white transition-all duration-300 transform hover:scale-105 shadow-md"
        >
          Comenzar el Repaso
        </Button>
      </CardContent>
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-6 bg-gradient-to-br from-sociologia-50 to-white rounded-xl shadow-2xl">
      <div className="mb-6 flex justify-between items-center">
        <Button onClick={onVolver} variant="outline" className="border-sociologia-400 text-sociologia-600 hover:bg-sociologia-100 transition-all duration-300 transform hover:scale-105 shadow-sm">
      ...
      {showInstructions ? renderInstructions() : (
        <>
          <AnimatePresence mode="wait">
            ...
            >
              <Card 
                className="w-full h-96 cursor-pointer bg-white shadow-2xl border-2 border-sociologia-300 hover:border-sociologia-500 transition-all duration-300 transform hover:scale-102 rounded-2xl overflow-hidden"
                onClick={flipCard}
              >
                <CardContent className="h-full flex flex-col items-center justify-center p-8 bg-gradient-to-br from-sociologia-50 to-sociologia-100">
                  <div 
                  ...
                  >
                    {isFlipped ? (
                      <CheckCircle className="text-sociologia-500 mb-6" size={48} />
                    ) : (
                      <Lightbulb className="text-sociologia-500 mb-6" size={48} />
                    )}
                    <h3 className="text-4xl font-bold text-sociologia-800 mb-6 leading-tight">
                      {isFlipped ? cards[currentCardIndex].definicion : cards[currentCardIndex].termino}
                    </h3>
                    <p className="text-sociologia-600 mt-4 bg-sociologia-200 px-4 py-2 rounded-full text-sm font-medium">
          ...
          <div className="flex justify-between">
            <Button onClick={prevCard} variant="outline" className="border-sociologia-400 text-sociologia-600 hover:bg-sociologia-100 transition-all duration-300 transform hover:scale-105 shadow-sm">
              <ArrowLeft className="mr-2 h-4 w-4" /> Anterior
            </Button>
            <Button onClick={shuffleCards} variant="default" className="bg-sociologia-500 hover:bg-sociologia-600 text-white transition-all duration-300 transform hover:scale-105 shadow-md">
              <Shuffle className="mr-2 h-4 w-4" /> Mezclar
            </Button>
            <Button onClick={nextCard} variant="outline" className="border-sociologia-400 text-sociologia-600 hover:bg-sociologia-100 transition-all duration-300 transform hover:scale-105 shadow-sm">
