...
import { CheckCircle, XCircle, ArrowRight, HelpCircle, Settings } from 'lucide-react';
  ...
  const renderConcepto = (concepto, tipo) => {
    ...
    return (
      ...
      >
        <Button
          variant="default"
          className={`w-full mb-2 py-3 px-4 transition-all duration-200 text-left justify-start
    ...
    <Card className="mb-4">
      ...
      <CardContent>
        ...
        <Card className="bg-sociologia-200 shadow-md">
          ...
          <CardContent>
            <p className="mb-4 text-sociologia-600">Selecciona el número de parejas a asociar:</p>
            <Slider
              value={[numeroParejas]}
              ...
              className="w-full"
            />
            <p className="mt-2 text-center text-2xl font-bold text-sociologia-700">{numeroParejas} parejas</p>
          </CardContent>
        </Card>
    ...
    return (
      <div className="w-full max-w-4xl mx-auto p-4 relative">
        ...
        <Card>
          ...
          <CardContent>
            <div className="mb-6">
              <p className="text-lg mb-2">Intentos exitosos: <span className="text-green-600 font-bold">{intentosExitosos}</span></p>
              <p className="text-lg mb-4">Intentos fallidos: <span className="text-red-600 font-bold">{intentosFallidos}</span></p>
            </div>
            <div className="space-y-4">
              <h3 className="text-xl font-semibold mb-2">Asociaciones correctas:</h3>
              {conceptos1.map((concepto, index) => (
                <Card key={index} className="p-4 bg-sociologia-100">
                  <div className="flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-4">
                    <span className="font-semibold text-sociologia-700">{concepto.concepto1}</span>
                    <ArrowRight className="w-6 h-6 text-sociologia-500" />
                    <span className="font-semibold text-sociologia-700">{concepto.concepto2}</span>
                  </div>
                  <p className="mt-2 text-sociologia-600 italic">Relación: {concepto.relacion}</p>
                </Card>
              ))}
            </div>
            <div className="mt-6 space-x-2 flex justify-center">
              <Button onClick={resetJuego} className="bg-sociologia-600 hover:bg-sociologia-700 text-white">
                Jugar de nuevo
              </Button>
          ...
          {mostrarInstrucciones ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <h3 className="text-lg font-semibold mb-2">Concepto 1</h3>
                  <AnimatePresence>
                    {conceptos1.map(concepto => renderConcepto(concepto, 'concepto1'))}
                  </AnimatePresence>
                </div>
                <div>
                  <h3 className="text-lg font-semibold mb-2">Concepto 2</h3>
                  <AnimatePresence>
